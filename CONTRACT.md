...
> **Status:** 🟢 ACTIVE — v0.8.0. Edge-only. Single binary, namespaced CLI: `do`/`admin`. 56 registered
...
| `window-create` | `Target.createTarget` (`newWindow: true`), optionally via `Target.createBrowserContext` (`private: true`) for incognito, then `Browser.getWindowForTarget` for the real `window_id`, then (optional) `layout` — see `## Window layout` | ❌ preflight `profile` + verify `url` | Deliberately NOT approval-gated. Optional `incognito: true` creates the whole window in a disposable InPrivate context |
...
| `page-press` | `Input.dispatchKeyEvent` (`keyDown` + `keyUp`) | ❌ | proper keyboard input via CDP Input domain — unlike `page-evaluate` + JS `KeyboardEvent` which only fires in the JS event system. Use for play/pause (`Space`), navigation (`ArrowDown`/`Enter`), shortcuts. Optional `modifier` bitfield (1=Alt, 2=Ctrl, 4=Meta, 8=Shift). |
...
| `page-screenshot` | `Page.captureScreenshot` | ❌ | payload optional `format` (`png`\|`jpeg`, default `png`), optional `output` (file path), optional `force_repaint` (default false — the old `translateZ(0)` hack was removed because it modified `document.body.style.transform` and broke SPAs like YouTube Music; pass `true` only as a last resort). When `output` is given, the decoded bytes are written to that local file and the RESULT carries `{"path": ...}`; otherwise the RESULT carries the base64 `data`. |
