...
smoke: ## Start an isolated daemon and prove a CLI Unix-socket roundtrip
	...
	@state="$$(mktemp -d)"; export BROWSER_PROXY_STATE_DIR="$$state" BROWSER_PROXY_EXTENSION_PORT=39491; uv run browser-proxy daemon >/dev/null 2>&1 & pid=$$!; for attempt in {1..50}; do [[ -S "$$state/browser-proxy.sock" ]] && break; sleep 0.1; done; 	uv run browser-proxy do profile-list '{}'; uv run browser-proxy admin service stop >/dev/null; wait $$pid; rmdir "$$state"
